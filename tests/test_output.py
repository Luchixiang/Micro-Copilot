from cellquant import io, models, metrics, plot, utils
...
def test_cli_2D(data_dir, image_names):
    ...
    chan2 = [1]
    for m, model_type in enumerate(model_types):
        cmd = [sys.executable, '-m', 'cellquant', '--dir', str(data_dir.joinpath('2D')),
               '--pretrained_model', model_type, '--no_resample', '--chan', str(chan[m]),
               '--chan2', str(chan2[m]), '--diameter', '0', '--no_interp', '--save_png', '--verbose']
        try:
            cmd_stdout = check_output(cmd, stderr=STDOUT).decode()
            print(cmd_stdout)
...
def test_cli_3D(data_dir, image_names):
    ...
    chan2 = [1]
    for m, model_type in enumerate(model_types):
        cmd = [sys.executable, '-m', 'cellquant', '--dir', str(data_dir.joinpath('3D')),
               '--do_3D', '--pretrained_model', model_type, '--no_resample',
               '--cellprob_threshold', '0', '--chan', str(chan[m]), '--chan2', str(chan2[m]),
               '--diameter', '25', '--save_tif']
        try:
            cmd_stdout = check_output(cmd, stderr=STDOUT).decode()
        except Exception as e:
            print(e)
