from cellquant import io, models, train, metrics, plot
...
def test_cli_train(data_dir):
    ...
    model_dir = str(data_dir.joinpath('2D').joinpath('train').joinpath('models'))
    shutil.rmtree(model_dir, ignore_errors=True)
    cmd = [sys.executable, '-m', 'cellquant', '--train', '--train_size', '--n_epochs', '3',
           '--dir', train_dir, '--mask_filter', '_cyto_masks', '--pretrained_model', 'None',
           '--chan', '2', '--chan2', '1', '--diam_mean', '40']
    try:
    ...
    cmd = [sys.executable, '-m', 'cellquant', '--dir', train_dir, '--pretrained_model',
           pretrained_model, '--chan', '2', '--chan2', '1', '--diam_mean', '40']
    try:
        cmd_stdout = check_output(cmd, stderr=STDOUT).decode()
    except Exception as e:
        print(e)
...
def test_cli_train_pretrained(data_dir):
    ...
    shutil.rmtree(model_dir, ignore_errors=True)
    cmd = [sys.executable, '-m', 'cellquant', '--train', '--train_size', '--n_epochs', '3',
           '--dir', train_dir, '--mask_filter', '_cyto_masks', '--pretrained_model', 'cyto',
           '--chan', '2', '--chan2', '1']
    try:
        cmd_stdout = check_output(cmd, stderr=STDOUT).decode()
    except Exception as e:
