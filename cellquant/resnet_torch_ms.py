...
class make_attention(nn.Module):
    def __init__(self, n_channels, conv_3D=False):
        super().__init__()
        self.convs = nn.ModuleList(
            [batchconv(n_channels[i], n_channels[i], sz=3, conv_3D=conv_3D) for i in range(len(n_channels))
             ])  # todo: use single conv or multiple convs

        self.max_pool = F.max_pool3d if conv_3D else F.max_pool2d
        self.upsample = F.upsample

    def forward(self, x0, rescale):
        x_out = []
        for i in range(len(x0)):
            x = x0[i].clone()
            if rescale > 1:
                x = self.upsample(x, scale_factor=rescale, mode='nearest')
            elif rescale < 1:
                x = self.max_pool(x, kernel_size=int(1 / rescale))
            x = self.convs[i](x)
            x = F.sigmoid(x)
...
class CPnetTwoScale(nn.Module):
    ...
    def __init__(self, nbase, nout, sz, mkldnn=False, conv_3D=False, max_pool=True,
        ...
        self.diam_labels = nn.Parameter(data=torch.ones(1) * diam_mean,
                                        requires_grad=False)
        # self.attentionout = nn.Sequential(nn.Conv2d(nbaseup[0] * 2, 1, 1), nn.Sigmoid())
        self.attentionout = nn.Sequential(batchconv(nbaseup[0] * 2, 1, 1), nn.Sigmoid())
        self.attentionout.apply(zero_init_weights) # zero init
    @property
    def device(self):
        """
    ...
    def forward(self, data):
        """
        Forward pass of the CPnetMultiScale model.
        ...
        img_scale0 = data
        img_scale1 = F.interpolate(img_scale0, scale_factor=1 / 2, mode='bilinear')
        if self.mkldnn:
            img_scale0 = img_scale0.to_mkldnn()
            img_scale1 = img_scale1.to_mkldnn()
        T0, style0 = self.encode(img_scale0)[:2]
        # print(T0[-1])
        T0_down, style1 = self.encode(img_scale1)[:2]
        T1, T1_feat = self.decode(T0, style0)
        T1_down, T1_down_feat = self.decode(T0_down, style1)
        T1_down_feat_rescale = F.interpolate(T1_down_feat, scale_factor=2, mode='bilinear')
        attention1 = self.attentionout(torch.cat([T1_down_feat_rescale, T1_feat], dim=1))
        # np.save('./attention.npy', (attention1 * 255).cpu().numpy().astype(np.uint8))
        feat = (1- attention1) * T1 + (attention1) * F.interpolate(T1_down, scale_factor=2, mode='bilinear')
        return F.interpolate(T1_down, scale_factor=2, mode='bilinear'), style0, T0
    ...
    def encode(self, data):
        ...
        if not self.style_on:
            style = style * 0
        return T0, style, style0

    def decode(self, T0, style):
        T1_feat = self.upsample(style, T0, self.mkldnn)
        # T1_attention = self.attentionout(T1)
        T1 = self.output(T1_feat)
        if self.mkldnn:
    ...
    def load_model(self, filename, device=None):
        ...
        if state_dict["output.2.weight"].shape[0] != self.nout:
            ...
            missing, unexcepted = self.load_state_dict(
                dict([(name, param) for name, param in state_dict.items()]),
                strict=False)
            print(f'loading missing key {missing}')
            print(f'loading unexcepted key {unexcepted}')
