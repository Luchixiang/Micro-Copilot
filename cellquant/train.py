...
import os
import numpy as np
from cellquant import io, transforms, utils, models, dynamics, metrics, resnet_torch
from cellquant.transforms import normalize_img
...
def WarmupCosineSchedule(optimizer, warmup_steps, t_total):
    """Cosine learning-rate schedule with a linear warm-up."""
    def lr_lambda(step):
...
def train_seg(net, train_data=None, train_labels=None, train_files=None,
              ...
              rescale=True, min_train_masks=5, model_name=None, multi_scale=False):
    """
    ...
    lavg, nsum = 0, 0
    total_iteration = n_epochs * (math.ceil(nimg / batch_size))
    warm_up_step = 10 * (math.ceil(nimg / batch_size))
    scheduler = WarmupCosineSchedule(optimizer, warmup_steps=warm_up_step, t_total=total_iteration)
    for iepoch in range(n_epochs):
        if multi_scale:
            if iepoch < n_epochs // 2:
                for param in net.attentionout.parameters():
