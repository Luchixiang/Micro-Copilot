...
WEIGHTS_DIR = PACKAGE_DIR.joinpath('weights')
CELLQUANT_MODEL_NAME = 'cellquant'


def default_cellpose_model_path():
    return os.environ.get(
        'CELLQUANT_CELLPOSE_MODEL',
        os.fspath(WEIGHTS_DIR.joinpath('model_0528_resize')),
    )


def default_puncta_model_path():
    return os.environ.get(
        'CELLQUANT_PUNCTA_MODEL',
        os.fspath(WEIGHTS_DIR.joinpath('model.pth')),
    )

from . import transforms, dynamics, utils, plot
from .resnet_torch import CPnet
from .core import assign_device, check_mkl, run_net, run_3D
from .resnet_torch_ms import CPnetTwoScale
...
class Cellpose():
    ...
    def __init__(self, gpu=False, model_type="cyto3", device=None, multi_scale=False):
        ...
        self.cp = CellposeModel(device=self.device, gpu=self.gpu, model_type=model_type,
                                diam_mean=self.diam_mean, multi_scale=multi_scale)
...
class CellposeModel():
    ...
    def __init__(self, gpu=False, pretrained_model=False, model_type=None,
                 diam_mean=30., device=None, nchan=2, multi_scale=False):
        """
        Initialize the CellposeModel.
        ...
        if multi_scale:
            self.net = CPnetTwoScale(self.nbase, self.nclasses, sz=3, mkldnn=self.mkldnn,
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
        ...
        Args:
            ...
            channels (list, optional): list of channels, either of length 2 or of length number of images by 2.
                First element of list is the channel to segment (0=grayscale, 1=Channel0, 2=Channel1, 3=Channel2, 4=Channel3).
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            if diameter is not None and diameter > 0:
                ...
                # rescale = self.diam_labels / diameter
