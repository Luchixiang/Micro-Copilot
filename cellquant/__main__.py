...
from cellquant import utils, models, io, version_str

from cellquant.cli import get_arg_parser

try:
    from cellquant.gui import gui3d, gui
    GUI_ENABLED = True
...
def main():
    ...
    if len(args.dir) == 0 and len(args.image_path) == 0:
        if args.add_model:
            ...
            if not GUI_ENABLED:
                ...
                if GUI_IMPORT:
                    ...
                    print("pip install 'cellquant[gui]'")
            else:
                if args.Zstack:
                    gui3d.run()
                else:
        ...
        if args.verbose:
            from cellquant.io import logger_setup

            logger, log_file = logger_setup()
        else:
            print(
                ">>>> !LOGGING OFF BY DEFAULT! To see Micro-copilot progress, set --verbose")
