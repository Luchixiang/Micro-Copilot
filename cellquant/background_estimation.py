...
def background_estimation(imgs, th = 1, dlevel = 7, wavename = 'db6', iter = 3):
    ''' Background estimation
        ...
        Background
    '''
    def decomposition_level(shape):
        return min(dlevel, pywt.dwtn_max_level(shape, wavename))
    ...
    try:
        ...
        for taxial in range(t):
            img = imgs[taxial, :, :]
            level = decomposition_level(img.shape)
            for i in range(iter):
                initial = img
                res = initial
                coeffs = pywt.wavedec2(res, wavelet = wavename, level = level)
                vec = Low_frequency_resolve(coeffs, level)
                ...
                if th > 0:
                    ...
                    coeffs1 = pywt.wavedec2(res, wavelet = wavename, level = level)
                    vec = Low_frequency_resolve(coeffs1, level)
                    Biter =  pywt.waverec2(vec, wavelet = wavename)
                    Biter_new = rm_1(Biter, x, y)
        ...
        level = decomposition_level(imgs.shape)
        for i in range(iter):
            initial = imgs
            res = initial
            coeffs = pywt.wavedec2(res, wavelet = wavename, level = level)
