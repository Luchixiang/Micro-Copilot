...
def _add_model(parent, filename=None, load_model=True):
    ...
    parent.ModelChooseB.addItems([fname])
    ...
    parent.ModelChooseB.setCurrentIndex(len(parent.model_strings))
    if load_model:
...
def _remove_model(parent, ind=None, verbose=True):
    if ind is None:
        ind = parent.ModelChooseB.currentIndex()
    if ind >= len(parent.net_names):
        # ind -= 1
        parent.ModelChooseB.removeItem(ind)
        del parent.model_strings[ind - len(parent.net_names)]
        # remove model from txt path
        modelstr = parent.ModelChooseB.currentText()
        remove_model(modelstr)
        if len(parent.model_strings) > 0:
            parent.ModelChooseB.setCurrentIndex(len(parent.net_names)+len(parent.model_strings) - 1)
        else:
            parent.ModelChooseB.setCurrentIndex(0)
    else:
...
def _load_image(parent, filename=None, load_seg=True, load_3D=False):
    ...
    if parent.loaded:
        ...
        initialize_images(parent, image, load_3D=load_3D)
        parent.loaded = True
...
def initialize_images(parent, image, load_3D=False):
    ...
    parent.nchan = 3
    logger.info(f'GUI INFO: load image of shape{image.shape}')#
    if image.ndim > 4:
        image = image.squeeze()
        if image.ndim > 4:
    ...
    if image.shape[-1] > 4:
        logger.info("WARNING: image has more than 4 channels, keeping only first 4")
        # image = image[..., :3]
        image = image[..., :4]
    elif image.shape[-1] == 3:
        shape = image.shape
        image = np.concatenate(
            (image, np.zeros((*shape[:-1], 4 - shape[-1]), dtype=np.uint8)), axis=-1)
        parent.nchan = 3
    ...
    parent.stack_unnorm = parent.stack.copy()
    ...
    if load_3D:
        logger.info("GUI_INFO: converted to float and normalized values to 0.0->255.0")
    ...
    if parent.autobtn.isChecked():
        if parent.restore is None or parent.restore != "filter":
            logger.info( "GUI_INFO: normalization checked: computing saturation levels (and optionally filtered image)")
...
def _masks_to_gui(parent, masks, outlines=None, colors=None):
    ...
    logger.info(f"GUI_INFO: {masks.max()} masks found")
    ...
    logger.info("GUI_INFO: creating cellcolors and drawing masks")
