...
from qtpy.QtWidgets import QScrollArea, QMainWindow, QApplication, QWidget, QScrollBar, QComboBox, QGridLayout, \
    QPushButton, QFrame, QCheckBox, QLabel, QProgressBar, QLineEdit, QMessageBox, QGroupBox, QSlider, QFileDialog
import pyqtgraph as pg
from .io import initialize_images
import numpy as np
...
# f = open(f'./logs/{folder_time}.txt', 'w')
# sys.stdout = f
...
from ..smart import SmartMicroscope
from ..analysis import PunctaDetectorProcess
from ..llm_config import load_llm_config, save_llm_config
from multiprocessing import Event
from ..writer_process import WriterProcess
from ..plot_process import PlotProcess
...
#     r = np.arange(0, 256)
#     color = np.zeros((256, 4))
#     color[:, cm] = r
#     color = color.astype(np.uint8)
#     cmap = pg.ColorMap(pos=np.linspace(0.0, 255, 256), color=color)
#     return cmap
def make_cmap(cm=0):
    # Define the range for the color intensity
    r = np.arange(0, 256)
    # Initialize the color array with zeros (256 colors, RGBA)
    color = np.zeros((256, 4), dtype=np.uint8)

    # Assign colors based on the specified colormap index
    if cm == 1:
        # Green
        color[:, 1] = r  # Only the green channel varies [[0, 255, 255], [0, 255, 0], [255, 0, 255], [255, 255, 0]]
    elif cm == 2:
        # Magenta (Red + Blue)
        color[:, 0] = r  # Red channel varies
        color[:, 2] = r  # Blue channel varies
    elif cm == 0:
        # Cyan (Green + Blue)
        color[:, 1] = r  # Green channel varies
        color[:, 2] = r  # Blue channel varies
    elif cm == 3:
        # Yellow (Red + Green)
        color[:, 0] = r  # Red channel varies
    ...
    # Create a ColorMap object with the generated color array
    cmap = pg.ColorMap(pos=np.linspace(0.0, 255, 256), color=color)
    return cmap
...
def run(image=None):
    ...
    packaged_icon = pathlib.Path(__file__).resolve().parent.parent.joinpath("logo", "logo.png")
    icon_path = packaged_icon if packaged_icon.is_file() else pathlib.Path.home().joinpath(".cellpose", "logo.png")
    ...
    ui = MainW(image=image, logger=logger)
    ui.win.show()
    ui.show()
...
class MainW(QMainWindow):
    ...
    def __init__(self, image=None, logger=None):
        ...
        self.setWindowTitle(f"Micro-copilot v{version}")
        ...
        self.lut_color = colors
        colornames = ["Cyan", "Green", "Magenta", "Yellow"]
        self.lut_color_names = colornames
        self.loaded = False
        ...
        self.agent = SmartMicroscope(self)
        self.done_event = Event()
        self.analyzer = PunctaDetectorProcess(self.queue, self.done_event, self.plot_queue, self.result_queue)
        self.analyzer.start()
        self.writer_process = WriterProcess(self.writer_queue)
        self.writer_process.start()
        self.plot_process = PlotProcess(self.plot_queue)
        ...
        self.lmain.addWidget(self.win, 0, 9, 40, 20)

        self.win.scene().sigMouseClicked.connect(self.plot_clicked)
        self.win.scene().sigMouseMoved.connect(self.mouse_moved)
        self.make_viewbox()
        # self.lmain.setColumnStretch(9, 2)
        bwrmap = make_bwr()
    ...
    @property
    def make_buttons(self):  # used to sett all the buttons on the gui
        ...
        b = 0
        b0 = 0
        self.imagingBox = QGroupBox("Smart Imaging")
        self.imagingBoxG = QGridLayout()
        self.imagingBox.setLayout(self.imagingBoxG)
        self.imagingBox.setFont(self.boldfont)
        self.l0.addWidget(self.imagingBox, b, 0, 1, 9)
        self.use_micromanager = QCheckBox("Use multi-d-acquisition")
        self.use_micromanager.setToolTip("Use the multi-dimensional acquisition settings from micromanager")
        self.use_micromanager.setFont(self.medfont)
        self.use_micromanager.setChecked(False)
        self.use_micromanager.stateChanged.connect(self.use_micromanager_changed)
        self.imagingBoxG.addWidget(self.use_micromanager, b0, 0, 1, 4)

        # the live cell imaging box
        b0 += 1
        self.timeBox = QGroupBox("Time lapse")
        self.timeBoxG = QGridLayout()
        self.timeBox.setLayout(self.timeBoxG)
        self.imagingBoxG.addWidget(self.timeBox, b0, 0, 3, 4)
        self.use_timelapse = QCheckBox("T acquisition")
        self.use_timelapse.setToolTip("Whether to live cell imaging")
        self.use_timelapse.setFont(self.medfont)
        self.use_timelapse.setChecked(False)
        self.use_timelapse.stateChanged.connect(self.use_time_imaging)
        self.timeBoxG.addWidget(self.use_timelapse, 0, 0, 1, 4)

        label = QLabel("Time duration (min):")
        label.setFont(self.medfont)
        label.setToolTip(
            'Set the total time duration of live cell imaging.'
        )
        self.timeBoxG.addWidget(label, 1, 0, 1, 4)

        self.time_duration = 60
        self.time_duration_text = QLineEdit()
        self.time_duration_text.setToolTip(
            'Set the total time duration of live cell imaging.'
        )
        self.time_duration_text.setText(str(self.time_duration))
        self.time_duration_text.setFont(self.medfont)
        self.time_duration_text.returnPressed.connect(self.update_time)
        self.time_duration_text.setFixedWidth(50)
        self.time_duration_text.setEnabled(False)
        self.timeBoxG.addWidget(self.time_duration_text, 1, 4, 1, 2)

        label = QLabel("Time interval (min):")
        label.setFont(self.medfont)
        label.setToolTip(
            'Set the time interval of live cell imaging.'
        )
        self.timeBoxG.addWidget(label, 2, 0, 1, 4)
        self.time_interval = 1
        self.time_interval_text = QLineEdit()
        self.time_interval_text.setToolTip(
            'Set the time interval of live cell imaging.'
        )
        self.time_interval_text.setText(str(self.time_interval))
        self.time_interval_text.setFont(self.medfont)
        self.time_interval_text.returnPressed.connect(self.update_time)
        self.time_interval_text.setFixedWidth(50)
        self.time_interval_text.setEnabled(False)
        self.timeBoxG.addWidget(self.time_interval_text, 2, 4, 1, 2)

        # z stack imaging box
        self.zStackBox = QGroupBox("Z-stack")
        self.zStackBoxG = QGridLayout()
        self.zStackBox.setLayout(self.zStackBoxG)
        self.imagingBoxG.addWidget(self.zStackBox, 0, 5, 4, 4)
        self.use_zstack = QCheckBox("Z acquisition")
        self.use_zstack.setToolTip("Whether to z-stack imaging")
        self.use_zstack.setFont(self.medfont)
        self.use_zstack.setChecked(False)
        self.use_zstack.stateChanged.connect(self.use_zstack_imaging)
        self.zStackBoxG.addWidget(self.use_zstack, 0, 0, 1, 4)

        label = QLabel("relative Z start (um):")
        label.setFont(self.medfont)
        label.setToolTip(
            'Set the relative z start distance.'
        )
        self.zStackBoxG.addWidget(label, 1, 0, 1, 4)

        self.z_start = 0
        self.z_start_label = QLineEdit()
        self.z_start_label.setText(str(self.z_start))
        self.z_start_label.setFont(self.medfont)
        self.z_start_label.setEnabled(False)

        self.zStackBoxG.addWidget(self.z_start_label, 1, 4, 1, 2)

        self.z_start_button = QPushButton('Set')
        self.z_start_button.clicked.connect(self.update_zstack_zstart)
        self.z_start_button.setEnabled(False)
        self.zStackBoxG.addWidget(self.z_start_button, 1, 6, 1, 2)

        label = QLabel("relative Z end (um):")
        label.setFont(self.medfont)
        label.setToolTip(
            'Set the relative z end distance.'
        )
        self.zStackBoxG.addWidget(label, 2, 0, 1, 4)

        self.z_end = 0
        self.z_end_label = QLineEdit()
        self.z_end_label.setText(str(self.z_end))
        self.z_end_label.setFont(self.medfont)
        self.z_end_label.setEnabled(False)

        self.zStackBoxG.addWidget(self.z_end_label, 2, 4, 1, 2)

        self.z_end_button = QPushButton('Set')
        # self.z_start_button.set
        self.z_end_button.setEnabled(False)
        self.zStackBoxG.addWidget(self.z_end_button, 2, 6, 1, 2)
        self.z_end_button.clicked.connect(self.update_zstack_zend)

        label = QLabel("Z step (um):")
        label.setFont(self.medfont)
        label.setToolTip(
            'Set the z step of z-stag imaging.'
        )
        self.zStackBoxG.addWidget(label, 3, 0, 1, 4)
        self.z_interval = 0.2
        self.z_interval_text = QLineEdit()
        self.z_interval_text.setToolTip(
            'Set the z step size of z-stack imaging.'
        )
        self.z_interval_text.setText(str(self.time_interval))
        self.z_interval_text.setFont(self.medfont)
        self.z_interval_text.setFixedWidth(50)
        self.z_interval_text.setEnabled(False)
        self.zStackBoxG.addWidget(self.z_interval_text, 3, 4, 1, 2)

        b0 += 3
        label = QLabel("Channel Group")
        label.setFont(self.medfont)
        label.setToolTip(
            'Choose the channel group of for imaging'
        )
        self.imagingBoxG.addWidget(label, b0, 0, 1, 4)
        self.channel_group_down = QComboBox()
        self.all_avaliable_groups = []
        # read all avaliable config group
        if hasattr(self.agent, 'core'):
            for i in range(self.agent.core.get_available_config_groups().size()):
                self.all_avaliable_groups.append((self.agent.core.get_available_config_groups().get(i)))
        self.channel_group_down.addItems(
            self.all_avaliable_groups)
        self.channel_group_down.setFont(self.medfont)
        self.channel_group_down.currentIndexChanged.connect(self.channel_group_changed)
        self.imagingBoxG.addWidget(self.channel_group_down, b0, 3, 1, 6)

        b0 += 1
        label = QLabel("Channels")
        label.setFont(self.medfont)
        label.setToolTip(
            'Choose the channels for imaging'
        )
        self.imagingBoxG.addWidget(label, b0, 0, 1, 4)
        b0 += 1
        self.all_avaliable_channels = ['None']

        self.imaging_channel_comboboxs = []
        self.use_channel2Judge_comboboxs = []
        self.imaging_exposure_comboboxs = []
        self.chosen_imaging_channel_list = set()
        self.prev_selected_imaging_channel = [None] * 4
        for i in range(4):
            label = QLabel(f'C{i + 1}')
            label.setFont(self.medfont)
            label.setStyleSheet(f"color: {self.lut_color_names[i]}")
            self.imagingBoxG.addWidget(label, b0, 0, 1, 1)
            imaging_channel1 = QComboBox()
            imaging_channel1.addItems(
                self.all_avaliable_channels)
            imaging_channel1.setFont(self.medfont)
            self.imaging_channel_comboboxs.append(imaging_channel1)
            imaging_channel1.currentIndexChanged.connect(
                lambda index, i=i: self.add_imaging_channel(i))
            self.imagingBoxG.addWidget(imaging_channel1, b0, 1, 1, 4)
            label = QLabel('exposure time (ms)')
            label.setFont(self.medfont)
            self.imagingBoxG.addWidget(label, b0, 5, 1, 1)
            imaging_exposure = QLineEdit()
            imaging_exposure.setText(str(200))
            imaging_exposure.setFont(self.medfont)
            imaging_exposure.setEnabled(False)

            self.imagingBoxG.addWidget(imaging_exposure, b0, 6, 1, 3)
            self.imaging_exposure_comboboxs.append(imaging_exposure)

            # self.use_channel2Judge1 = QCheckBox('Must contain this information')
            # self.use_channel2Judge1.setToolTip('Whether the cell found must contains this channel information')
            # self.use_channel2Judge1.setChecked(False)
            # self.use_channel2Judge1.setFont(self.medfont)
            # # self.use_channel2detect1.stateChanged.connect(self.use_micromanager_changed)  # todo: disable all other chooses
            # self.imagingBoxG.addWidget(self.use_channel2Judge1, b0, 5, 1, 4)
            # self.imaging_channel_comboboxs.append(self.imaging_channel1)
            # self.use_channel2Judge_comboboxs.append(self.use_channel2Judge1)
            b0 += 1
        self.channel_group_changed()
        label = QLabel('File Directory:')
        label.setFont(self.medfont)
        self.imagingBoxG.addWidget(label, b0, 0, 1, 2)
        self.file_directory_text = QLineEdit()
        self.file_directory_text.setFont(self.medfont)
        self.imagingBoxG.addWidget(self.file_directory_text, b0, 2, 1, 6)
        self.file_directory_button = QPushButton()
        self.file_directory_button.setText('...')
        self.file_directory_button.setFont(self.medfont)
        self.file_directory_button.pressed.connect(self.choose_file_directory)
        self.imagingBoxG.addWidget(self.file_directory_button, b0, 8, 1, 1)
        ...
        self.file_name_text.setFont(self.medfont)
        self.imagingBoxG.addWidget(self.file_name_text, b0, 2, 1, 7)

        b0 += 1
        label = QLabel("Chan to find cell")
        label.setFont(self.medfont)
        label.setToolTip(
            'Choose the channels to find the cell'
        )
        self.imagingBoxG.addWidget(label, b0, 0, 1, 4)

        self.find_cell_combox = QComboBox()
        self.find_cell_combox.setFont(self.medfont)
        # self.find_cell_combox.keyPressEvent.connect(self.read_channels_from_mm)
        self.imagingBoxG.addWidget(self.find_cell_combox, b0, 3, 1, 6)

        b0 += 1
        label = QLabel("Cell numbers")
        label.setFont(self.medfont)
        label.setToolTip(
            'The cell numbers to find. May not meet if the time interval is too shor.'
        )
        self.imagingBoxG.addWidget(label, b0, 0, 1, 2)

        self.cellNumberEdit = QLineEdit()
        self.cellNumberEdit.setToolTip(
            'the number of cell to capture and analyze. Make sure the pycromanager is installed and connected to the microscope. '
        )
        self.cellNumber = 20
        self.cellNumberEdit.setText(str(self.cellNumber))
        self.cellNumberEdit.setFont(self.medfont)
        # self.cellNumberEdit.returnPressed.connect(self.update_cell_number) # todo: update cell numbers
        self.cellNumberEdit.setFixedWidth(50)
        self.imagingBoxG.addWidget(self.cellNumberEdit, b0, 2, 1, 1)
        self.ImagingButton = QPushButton("Acquire")
        self.ImagingButton.setFont(self.boldfont)
        self.ImagingButton.clicked.connect(self.automatic_imaging)
        self.imagingBoxG.addWidget(self.ImagingButton, b0, 3, 1, 4)
        self.abortButton = QPushButton("Abort")
        self.abortButton.setEnabled(False)
        self.abortButton.setFont(self.boldfont)
        self.abortButton.clicked.connect(self.abort_imaging)
        self.imagingBoxG.addWidget(self.abortButton, b0, 7, 1, 2)
        # self.SizeButton.setFixedWidth(65)

        self.ImagingButton.setToolTip(
            'Automatic Imaging. Make sure the pycromanager is installed and connected to the microscope.'
        )
        # if hasattr(self.agent, 'core'):
        #     self.imagingBox.setEnabled(True)
        # else:
        #     self.imagingBox.setEnabled(False)
        b += 1
        b0 = 0
        ...
        self.ChannelChoose.addItems(["0:gray", "C1", "C2", "C3", "C4"])
        cstr = "chan to segment:"
        ...
        b0 += 1
        self.net_names = [
            models.CELLQUANT_MODEL_NAME, "cyto3", "nuclei", "cyto2_cp3", "tissuenet_cp3", "livecell_cp3", "yeast_PhC_cp3",
            "yeast_BF_cp3", "bact_phase_cp3", "bact_fluor_cp3", "deepbacs_cp3"
        ]
        nett = [
            "Micro-copilot (bundled)", "cyto3", "nuclei", "cellpose (cyto2_cp3)", "tissuenet_cp3", "livecell_cp3",
            "yeast_PhC_cp3", "yeast_BF_cp3", "bact_phase_cp3", "bact_fluor_cp3",
            "deepbacs_cp3"
        ]
        b0 += 1
        label = QLabel('Segmentation model')
        label.setFont(self.medfont)
        self.segBoxG.addWidget(label, b0, 0, 1, 4)
        self.ModelChooseB = QComboBox()
        self.ModelChooseB.setFont(self.medfont)
        self.ModelChooseB.addItems(nett)
        self.ModelChooseB.addItems(self.model_strings)
        # self.ModelChooseB.setFixedWidth(175)
        tipstr = "dataset-specific models"
        self.ModelChooseB.setToolTip(tipstr)
        # self.ModelChooseB.activated.connect(lambda: self.model_choose())
        self.ModelChooseB.currentIndexChanged.connect(lambda: self.model_choose())
        self.segBoxG.addWidget(self.ModelChooseB, b0, 4, 1, 5)
        ...
        nett = ["run segmentation"]
        self.StyleButtons = []
        jj = 4
        self.run_segmentation_button = QPushButton()
        self.run_segmentation_button.setText('Run Segmentation')
        self.run_segmentation_button.setEnabled(False)
        self.run_segmentation_button.setFont(self.boldfont)
        self.segBoxG.addWidget(self.run_segmentation_button, b0, 4, 1, 5)
        self.run_segmentation_button.pressed.connect(self.cell_segmentation_resize)
        ...
        b += 1
        self.analysisBox = QGroupBox("Analysis")
        ...
        b0 = 0
        self.analysis_checkbox = QCheckBox("Real time analysis")
        self.analysis_checkbox.setToolTip("Do real-time analysis (e.g., puncta_detection)")
        self.analysis_checkbox.setFont(self.medfont)
        self.analysis_checkbox.setChecked(True)
        # self.analysis_checkbox.stateChanged.connect(self.use_micromanager_changed)  # todo: diseable all other buttons
        self.analysisBoxG.addWidget(self.analysis_checkbox, b0, 0, 1, 2)

        b0 += 1
        label = QLabel("Analysis instruction")
        label.setFont(self.medfont)
        label.setToolTip(
            "Describe what to analyze and which channel to use. Example: segment nucleus in first channel; detect lysosome in second and third channel; calculate localization ratio of channel2 in channel3."
        )
        self.analysisBoxG.addWidget(label, b0, 0, 1, 3)
        self.analysis_instruction_text = QLineEdit()
        self.analysis_instruction_text.setFont(self.medfont)
        self.analysis_instruction_text.setPlaceholderText(
            "segment nucleus in first channel; detect lysosome in second and third channel; calculate localization ratio of channel2 in channel3"
        )
        self.analysis_instruction_text.setToolTip(
            "Leave empty to use the fixed channel selectors below. Mention lysosome to use the Micro-copilot lysosome detector; other structures use LLM-generated Python or FIJI code."
        )
        self.analysisBoxG.addWidget(self.analysis_instruction_text, b0, 3, 1, 4)
        self.use_language_instruction = QCheckBox("Use language")
        self.use_language_instruction.setFont(self.medfont)
        self.use_language_instruction.setChecked(False)
        self.use_language_instruction.setToolTip(
            "Off: use fixed GUI channel selectors for lysosome detection. On: parse the language instruction with LLM/local parser."
        )
        self.analysisBoxG.addWidget(self.use_language_instruction, b0, 7, 1, 2)

        b0 += 1
        label = QLabel("Custom analysis code")
        label.setFont(self.medfont)
        label.setToolTip(
            "For non-Micro-copilot tools, choose whether the LLM writes a reusable Python script or a reusable FIJI macro once per experiment."
        )
        self.analysisBoxG.addWidget(label, b0, 0, 1, 3)
        self.custom_analysis_mode = QComboBox()
        self.custom_analysis_mode.addItems(["LLM Python script", "LLM FIJI macro"])
        self.custom_analysis_mode.setFont(self.medfont)
        self.custom_analysis_mode.setToolTip(
            "Python scripts run inside Micro-copilot. FIJI macros are saved and run headless when FIJI_PATH or IMAGEJ_PATH points to Fiji/ImageJ."
        )
        self.analysisBoxG.addWidget(self.custom_analysis_mode, b0, 3, 1, 6)

        b0 += 1
        llm_config = load_llm_config()
        label = QLabel("LLM endpoint")
        label.setFont(self.medfont)
        label.setToolTip("Azure OpenAI endpoint used by custom analysis and Co-Scientists dialog.")
        self.analysisBoxG.addWidget(label, b0, 0, 1, 2)
        self.llm_endpoint_text = QLineEdit()
        self.llm_endpoint_text.setFont(self.medfont)
        self.llm_endpoint_text.setText(llm_config.get('endpoint', ''))
        self.analysisBoxG.addWidget(self.llm_endpoint_text, b0, 2, 1, 4)
        label = QLabel("model")
        label.setFont(self.medfont)
        self.analysisBoxG.addWidget(label, b0, 6, 1, 1)
        self.llm_model_text = QLineEdit()
        self.llm_model_text.setFont(self.medfont)
        self.llm_model_text.setText(llm_config.get('model', 'gpt-4o'))
        self.analysisBoxG.addWidget(self.llm_model_text, b0, 7, 1, 2)

        b0 += 1
        label = QLabel("LLM API key")
        label.setFont(self.medfont)
        label.setToolTip("API key used by custom analysis and Co-Scientists dialog.")
        self.analysisBoxG.addWidget(label, b0, 0, 1, 2)
        self.llm_api_key_text = QLineEdit()
        self.llm_api_key_text.setFont(self.medfont)
        self.llm_api_key_text.setEchoMode(QLineEdit.Password)
        self.llm_api_key_text.setText(llm_config.get('api_key', ''))
        self.analysisBoxG.addWidget(self.llm_api_key_text, b0, 2, 1, 3)
        label = QLabel("API version")
        label.setFont(self.medfont)
        self.analysisBoxG.addWidget(label, b0, 5, 1, 1)
        self.llm_api_version_text = QLineEdit()
        self.llm_api_version_text.setFont(self.medfont)
        self.llm_api_version_text.setText(llm_config.get('api_version', '2024-10-21'))
        self.analysisBoxG.addWidget(self.llm_api_version_text, b0, 6, 1, 2)
        self.llm_save_button = QPushButton('Save')
        self.llm_save_button.setFont(self.medfont)
        self.llm_save_button.clicked.connect(self.save_current_llm_config)
        self.llm_save_button.setToolTip('Save LLM settings for custom analysis and Co-Scientists dialog.')
        self.analysisBoxG.addWidget(self.llm_save_button, b0, 8, 1, 1)

        b0 += 1
        label = QLabel("puncta threshold ")

        label.setFont(self.medfont)
        label.setToolTip(
            'Use the lower threshold to detect more punctas'
        )
        self.analysisBoxG.addWidget(label, b0, 3, 1, 1)
        self.punctaThresh = 0.2
        self.punctaThreshText = QLineEdit()
        self.punctaThreshText.setText(str(self.punctaThresh))
        self.punctaThreshText.setFixedWidth(40)
        self.punctaThreshText.setFont(self.medfont)
        self.punctaThreshText.setToolTip(
            "NOTE: You can lower the threshold to detect more punctas "
        )
        self.analysisBoxG.addWidget(self.punctaThreshText, b0, 4, 1, 2)
        label = QLabel("co-distance threshold(pix) ")

        label.setFont(self.medfont)
        label.setToolTip(
            'The maximum distance between two puncta that be regarded as co. Can be minus'
        )
        self.analysisBoxG.addWidget(label, b0, 6, 1, 1)
        self.punctaDistText = QLineEdit()
        self.punctaDistText.setText('-2')
        self.punctaDistText.setFixedWidth(40)
        self.punctaDistText.setFont(self.medfont)
        self.punctaDistText.setToolTip(
            'The maximum distance between two puncta that be regarded as co. Can be minus'
        )
        self.analysisBoxG.addWidget(self.punctaDistText, b0, 7, 1, 2)
        #
        b0 += 1
        # choose channel for blob detection
        self.punctaChannelChoose = [QComboBox(), QComboBox(), QComboBox(), QComboBox()]
        self.punctaChannelChoose[0].addItems(["Channel 1", "Channel 2", "Channel 3", "Channel 4"])
        self.punctaChannelChoose[1].addItems(["None", "Channel 1", "Channel 2", "Channel 3", "Channel 4"])
        self.punctaChannelChoose[2].addItems(["None", "Channel 1", "Channel 2", "Channel 3", "Channel 4"])
        self.punctaChannelChoose[3].addItems(["None", "Channel 1", "Channel 2", "Channel 3", "Channel 4"])
        cstr = ["Detect structure in chan:", "Detect structure in chan (optional): ",
                "Detect structure in chan (optional):", "nuclear chan for nuclear distance (optional):"]
        for i in range(4):
            self.punctaChannelChoose[i].setFont(self.medfont)
            self.punctaChannelChoose[i].currentIndexChanged.connect(self.analysis_channel_change)
            label = QLabel(cstr[i])
            label.setFont(self.medfont)
            if i == 3:
                label.setToolTip(
                    "The channel in which the nuclear are segmented. Please specify it if you want to calculate the distance between structure and nuclear"
                )
                self.punctaChannelChoose[i].setToolTip(
                    "The channel in which the nuclear are segmented.  Please specify it if you want to calculate the distance between structure and nuclear"
                )
            else:
                label.setToolTip(
                    "The channel in which you want to detect structure. If multiple channels are specified, their co-localization will be calculated."
                )
                self.punctaChannelChoose[i].setToolTip(
                    "The channel in which you want to detect structure. If multiple channels are specified, their co-localization will be calculated."
                )

            self.analysisBoxG.addWidget(label, b0 + i, 0, 1, 4)
            self.analysisBoxG.addWidget(self.punctaChannelChoose[i], b0 + i, 4, 1, 5)

        b0 += 4

        # Whether to use deep learning to remove possible FP
        self.useDL = QCheckBox("use DL")
        self.useDL.setToolTip(
            "Use the bundled puncta classifier to remove possible false positives and merge rings."
        )
        self.useDL.setFont(self.medfont)
        classifier_path = models.default_puncta_model_path()
        self.useDL.setChecked(bool(classifier_path and os.path.isfile(classifier_path)))
        self.analysisBoxG.addWidget(self.useDL, b0, 0, 1, 3)
        # compute segmentation with general models
        self.puncta_text = ["Run:"]
        puncta_net_text = ['Run ']
        self.punctaStyleButtons = []
        jj = 3
        for j in range(len(self.puncta_text)):
            self.punctaStyleButtons.append(
                guiparts.punctaModelButton(self, puncta_net_text[j]))
            w = 2
            self.analysisBoxG.addWidget(self.punctaStyleButtons[-1], b0, jj, 1, w)
            self.analysisBoxG.addWidget(self.punctaStyleButtons[-1], b0, jj, 1, w)
            jj += w
            self.punctaStyleButtons[-1].setToolTip(
                'run structure detection and co-localization if two channel were chosen')
        self.analyzeImageFileButton = QPushButton('Analyze file')
        self.analyzeImageFileButton.setFont(self.boldfont)
        self.analyzeImageFileButton.clicked.connect(self.analyze_selected_image_file)
        self.analyzeImageFileButton.setToolTip('Select and load one image file. Click Run to analyze it without smart imaging.')
        self.analysisBoxG.addWidget(self.analyzeImageFileButton, b0, jj, 1, 2)
        jj += 2
        self.analyzeImageFolderButton = QPushButton('Analyze folder')
        self.analyzeImageFolderButton.setFont(self.boldfont)
        self.analyzeImageFolderButton.clicked.connect(self.analyze_selected_image_folder)
        self.analyzeImageFolderButton.setToolTip('Select an image folder. Click Run to analyze all TIFF images without smart imaging.')
        self.analysisBoxG.addWidget(self.analyzeImageFolderButton, b0, jj, 1, 2)

        b0 += 1
        label = QLabel('Real time figure X property and channel')
        label.setFont(self.medfont)
        label.setToolTip(
            'One real time figure can be created. Please specify the X axis structure property and channel.')
        self.analysisBoxG.addWidget(label, b0, 0, 1, 2)
        self.figure_x_combobox_property = QComboBox()
        self.figure_x_combobox_property.addItems(['none', 'time'] + self.analyzer.organelle_properties[:-1])
        self.figure_x_combobox_property.setFont(self.medfont)
        self.figure_x_combobox_property.currentIndexChanged.connect(self.figure_x_change)
        self.figure_x_combobox_property.setToolTip(
            'One real time figure can be created. Please specify the X axis property.')
        # self.figure_x_combobox_property.setFixedWidth()
        self.analysisBoxG.addWidget(self.figure_x_combobox_property, b0, 3, 1, 3)
        self.figure_x_combobox_chan = QComboBox()
        # self.figure_x_combobox_chan.addItems(self.analyzer.organelle_chans)
        self.figure_x_combobox_chan.setFont(self.medfont)
        self.figure_x_combobox_chan.setToolTip(
            "Please specify the X axis channel.The 'in' means co-localize")
        self.analysisBoxG.addWidget(self.figure_x_combobox_chan, b0, 6, 1, 3)
        b0 += 1
        label = QLabel('Real time figure Y property and channel')
        label.setFont(self.medfont)
        label.setToolTip(
            'One real time figure can be created. Please specify the Y axis organelle property and channel.')
        self.analysisBoxG.addWidget(label, b0, 0, 1, 2)
        self.figure_y_combobox_property = QComboBox()
        self.figure_y_combobox_property.addItems(['none'] + self.analyzer.organelle_properties)
        self.figure_y_combobox_property.setFont(self.medfont)
        self.figure_y_combobox_property.currentIndexChanged.connect(self.figure_y_change)
        self.figure_y_combobox_property.setToolTip(
            'One real time figure can be created. Please specify the Y axis property.')
        self.analysisBoxG.addWidget(self.figure_y_combobox_property, b0, 3, 1, 3)
        self.figure_y_combobox_chan = QComboBox()
        # self.figure_y_combobox_chan.addItems(self.analyzer.organelle_chans)
        self.figure_y_combobox_chan.setFont(self.medfont)
        self.figure_y_combobox_chan.setToolTip(
            'One real time figure can be created. Please specify the Y axis channel.')
        self.analysisBoxG.addWidget(self.figure_y_combobox_chan, b0, 6, 1, 3)
        ...
        self.RGBDropDown.addItems(
            ["Composite", "C1", "C2", "C3", "C4", "gray", "spectral"])
        ...
        for r in range(4):
            b0 += 1
            if r == 0:
                label = QLabel('<font color="gray">gray/</font><br>C1')
            else:
                label = QLabel(names[r] + ":")
            label.setStyleSheet(f"color: {self.lut_color_names[r]}")
            label.setFont(self.boldmedfont)
            self.satBoxG.addWidget(label, b0, 0, 1, 2)
            self.sliders.append(Slider(self, names[r], self.lut_color[r]))
    ...
    def use_micromanager_changed(self):
        if self.use_micromanager.isChecked():
            self.timeBox.setEnabled(False)
            self.zStackBox.setEnabled(False)
            self.channel_group_down.setEnabled(False)
            for i in range(len(self.imaging_channel_comboboxs)):
                self.imaging_channel_comboboxs[i].setEnabled(False)
                self.imaging_exposure_comboboxs[i].setEnabled(False)
            # self.find_cell_combox.setEnabled(False)
            self.file_directory_button.setEnabled(False)
            self.file_directory_text.setEnabled(False)
            self.file_name_text.setEnabled(False)
            if hasattr(self.agent, 'core') and hasattr(self, 'find_cell_combox'):
                self.find_cell_combox.clear()
                channels = self.agent.studio.acquisitions().get_acquisition_settings().channels()
                if channels.size() > 0:
                    for i in range(channels.size()):
                        self.find_cell_combox.addItem(channels.get(i).config())
                # self.find_cell_combox.addItems(channels)
        else:
            self.timeBox.setEnabled(True)
            self.zStackBox.setEnabled(True)
            self.channel_group_down.setEnabled(True)
            for i in range(len(self.imaging_channel_comboboxs)):
                self.imaging_channel_comboboxs[i].setEnabled(True)
                self.imaging_exposure_comboboxs[i].setEnabled(True)
            self.find_cell_combox.setEnabled(True)
            self.file_directory_button.setEnabled(True)
            self.file_directory_text.setEnabled(True)
            self.file_name_text.setEnabled(True)

    def use_time_imaging(self):
        if self.use_timelapse.isChecked():
            self.time_duration_text.setEnabled(True)
            self.time_interval_text.setEnabled(True)
        else:
            self.time_duration_text.setEnabled(False)
            self.time_interval_text.setEnabled(False)

    def use_zstack_imaging(self):
        if self.use_zstack.isChecked():
            self.z_start_button.setEnabled(True)
            self.z_end_button.setEnabled(True)
            self.z_start_label.setEnabled(True)
            self.z_end_label.setEnabled(True)
            self.z_interval_text.setEnabled(True)
        else:
            self.z_start_button.setEnabled(False)
            self.z_end_button.setEnabled(False)
            self.z_start_label.setEnabled(False)
    ...
    def channel_group_changed(self):
        self.prev_selected_imaging_channel = [None] * 4
        self.chosen_imaging_channel_list = set()
        for i in range(4):
            self.imaging_channel_comboboxs[i].clear()
        self.all_avaliable_channels = ['None']
        self.channel_group_name = self.channel_group_down.currentText()
        if hasattr(self.agent, 'core'):
            for i in range(self.agent.core.get_available_configs(self.channel_group_name).size()):
                self.all_avaliable_channels.append(
                    (self.agent.core.get_available_configs(self.channel_group_name).get(i)))
        for i in range(4):
            for j in range(len(self.all_avaliable_channels)):
                self.imaging_channel_comboboxs[i].addItem(self.all_avaliable_channels[j])
        for i in range(4):
            self.imaging_channel_comboboxs[i].setCurrentIndex(0)

    def add_imaging_channel(self, current_combobox_index):
        if self.prev_selected_imaging_channel[current_combobox_index] is not None:
            self.chosen_imaging_channel_list.remove(self.prev_selected_imaging_channel[current_combobox_index])
            for i, combobox in enumerate(self.imaging_channel_comboboxs):
                if i != current_combobox_index:
                    combobox.addItem(self.prev_selected_imaging_channel[current_combobox_index])

        chosen_item = self.imaging_channel_comboboxs[current_combobox_index].currentText()
        if self.imaging_channel_comboboxs[current_combobox_index].currentIndex() > 0:
            self.imaging_exposure_comboboxs[current_combobox_index].setEnabled(True)
            self.prev_selected_imaging_channel[current_combobox_index] = chosen_item
            self.chosen_imaging_channel_list.add(chosen_item)
            # self.imaging_channel_comboboxs[current_combobox_index].ind
            for i, combobox in enumerate(self.imaging_channel_comboboxs):
                if i != current_combobox_index:
                    combobox.removeItem(combobox.findText(chosen_item))
        else:
            self.imaging_exposure_comboboxs[current_combobox_index].setEnabled(False)
            self.prev_selected_imaging_channel[current_combobox_index] = None

        if hasattr(self, 'find_cell_combox'):
            self.find_cell_combox.clear()
            self.find_cell_combox.addItems(self.chosen_imaging_channel_list)

    def figure_y_change(self):
        option = self.figure_y_combobox_property.currentText()
        self.figure_y_combobox_chan.clear()
        selected_chans = []
        for box in self.punctaChannelChoose[:-1]:
            if box.currentText() != 'None':
                selected_chans.append(int(box.currentText().split(' ')[1]))
        if option == 'none':
            self.figure_y_combobox_chan.addItems([])  # none
        elif option == 'structure intensity' or option == 'structure number':  # all
            for i in selected_chans:
                self.figure_y_combobox_chan.addItem(f'C{i}')  # not co
                for j in selected_chans:
                    if j == i:
                        continue
                    self.figure_y_combobox_chan.addItem(f'C{i} in C{j}')  # co

        elif option == 'co-localization ratio':  # only co
            for i in selected_chans:
                for j in selected_chans:
                    if j == i:
                        continue
                    self.figure_y_combobox_chan.addItem(f'C{i} in C{j}')  # none
        else:  # only not co
            for i in selected_chans:
                self.figure_y_combobox_chan.addItem(f'C{i}')  # not co

    def figure_x_change(self):  # three places: here, table, plot
        option = self.figure_x_combobox_property.currentText()
        self.figure_x_combobox_chan.clear()
        selected_chans = []
        for box in self.punctaChannelChoose[:-1]:
            if box.currentText() != 'None':
                selected_chans.append(int(box.currentText().split(' ')[1]))
        if option == 'none' or option == 'time':  # none
            self.figure_x_combobox_chan.addItems([])
        else:
            for i in selected_chans:
                self.figure_x_combobox_chan.addItem(f'C{i}')  # not co

    def analysis_channel_change(self):
        self.figure_x_combobox_property.setCurrentText('none')
        self.figure_y_combobox_property.setCurrentText('none')

    def choose_file_directory(self):
        qdialog = QFileDialog()
        # qdialog.setFileMode(QFileDialog.)
        # self.imagingBoxG.addWidget(qdialog)
        directory = QFileDialog.getExistingDirectory(None, "Select a folder", options=QFileDialog.ShowDirsOnly)
        if directory is not None and os.path.exists(directory):
            self.file_directory_text.setText(directory)

    def get_puncta_detection_channels(self):
        chan0 = self.punctaChannelChoose[0].currentIndex()
        chan1 = self.punctaChannelChoose[1].currentIndex() - 1
        chan2 = self.punctaChannelChoose[2].currentIndex() - 1
        chan3 = self.punctaChannelChoose[3].currentIndex() - 1
        if chan0 >= self.nchan:
            chan0 = 0
        if chan1 >= self.nchan:
            chan1 = -1
    ...
    def get_analysis_instruction(self):
        if hasattr(self, 'use_language_instruction') and not self.use_language_instruction.isChecked():
            return ''
        if hasattr(self, 'analysis_instruction_text'):
            return self.analysis_instruction_text.text().strip()
        return ''

    def get_analysis_instruction_text(self):
        if hasattr(self, 'analysis_instruction_text'):
            return self.analysis_instruction_text.text().strip()
        return ''

    def get_custom_analysis_mode(self):
        if hasattr(self, 'custom_analysis_mode'):
            return self.custom_analysis_mode.currentText()
        return 'LLM Python script'

    def get_current_llm_config(self):
        return {
            'api_key': self.llm_api_key_text.text() if hasattr(self, 'llm_api_key_text') else '',
            'endpoint': self.llm_endpoint_text.text() if hasattr(self, 'llm_endpoint_text') else '',
            'api_version': self.llm_api_version_text.text() if hasattr(self, 'llm_api_version_text') else '2024-10-21',
            'model': self.llm_model_text.text() if hasattr(self, 'llm_model_text') else 'gpt-4o',
        }

    def safe_float_text(self, widget, default, name):
        try:
    ...
    def safe_int_text(self, widget, default, name):
        try:
    ...
    def save_current_llm_config(self):
        try:
            path = save_llm_config(self.get_current_llm_config())
            print(f"LLM config saved to {path}")
            return path
        except Exception as e:
            print(f"WARNING: failed to save LLM config: {e}")
            return None

    def open_gpt_dialog(self):
        self.save_current_llm_config()
        from cellquant.gui.gpt_dialog import GPTDialogWindow
        if not hasattr(self, 'gpt_dialog_window') or self.gpt_dialog_window is None:
            self.gpt_dialog_window = GPTDialogWindow(parent=self)
        self.gpt_dialog_window.show()
        self.gpt_dialog_window.raise_()
        self.gpt_dialog_window.activateWindow()

    def puncta_detection(self):
        if not getattr(self, 'loaded', False) and not (hasattr(self, 'pending_analysis_folder') and self.pending_analysis_folder):
            print('WARNING: no image loaded. Please load/select an image or folder before clicking Run.')
            return
        if hasattr(self, 'pending_analysis_folder') and self.pending_analysis_folder and not getattr(self, 'processing_analysis_folder', False):
            directory = self.pending_analysis_folder
            self.pending_analysis_folder = None
            self.analyze_image_folder(directory)
            return
        self.analysisBox.setEnabled(False)
        self.segBox.setEnabled(False)
        self.done_event.clear()
        try:
            cell_masks = self.cell_segmentation_resize()
        except Exception as e:
            print(f'WARNING: cell segmentation failed before analysis, using analysis fallback: {e}')
            cell_masks = None
        draw_x = self.figure_x_combobox_chan.currentText() + ' ' + self.figure_x_combobox_property.currentText()
        draw_y = self.figure_y_combobox_chan.currentText() + ' ' + self.figure_y_combobox_property.currentText()
        if self.restore is not None and self.restore != "filter":
            img = self.stack_filtered.copy().squeeze()
        else:
            # img = self.stack.copy().squeeze()
            img = self.stack_unnorm.copy().squeeze()
        chan0, chan1, chan2, chan3 = self.get_puncta_detection_channels()
        analysis_instruction = self.get_analysis_instruction()
        custom_analysis_mode = self.get_custom_analysis_mode()
        segmentation_record = self.agent.get_segmentation_record(mask=None) if hasattr(self.agent, 'get_segmentation_record') else None
        threshold = self.safe_float_text(self.punctaThreshText, 0.2, 'puncta threshold')
        distance_threshold = self.safe_int_text(self.punctaDistText, -2, 'co-distance threshold')
        self.queue.put((img.astype(np.uint16), cell_masks, self.filename, None,
                        (chan0, chan1, chan2, chan3),
                        threshold, distance_threshold,
                        self.useDL.isChecked(), -1, draw_x, draw_y, analysis_instruction, custom_analysis_mode,
                        segmentation_record))
        self.done_event.wait()
        try:
            detection_result, colocalization_result = self.result_queue.get(timeout=1)
        except Empty:
            detection_result = None

        if detection_result is not None:
            # chan1 = self.punctaChannelChoose[0].currentIndex()
            # chan2 = self.punctaChannelChoose[1].currentIndex() - 1
            # chan3 = self.punctaChannelChoose[2].currentIndex() - 1
            if img.ndim == 3:

                puncta_masks = np.zeros_like(img[:, :, 0])
            else:
                puncta_masks = np.zeros_like(img)
            colors = []
            for chan in [chan0, chan1, chan2]:
                if chan >= 0 and chan in detection_result.keys():
                    colors.append(self.lut_color[chan])
                    blobs = detection_result[chan][0]
                    for blob in blobs:
                        for y, x, r in blob:
                            rr, cc = sk_disk((int(y), int(x)), int(r * 1.4), shape=puncta_masks.shape)
                            puncta_masks[rr, cc] = chan + 1

            if colors:
                if self.NZ == 1:
                    puncta_masks = puncta_masks[np.newaxis, ...]
                io._masks_to_gui(self, puncta_masks.astype(np.uint8), colors=colors)
            else:
                print('No lysosome detections were returned for the selected channels.')
        else:
            print('No analysis result was returned.')

        # Keep the cell segmentation visible after analysis. The previous code
        # replaced it with a puncta-only label image at the end of the run.
        if cell_masks is not None:
            displayed_cell_masks = np.asarray(cell_masks)
            if displayed_cell_masks.ndim == 2:
                displayed_cell_masks = displayed_cell_masks[np.newaxis, ...]
            io._masks_to_gui(self, displayed_cell_masks, outlines=None)
            self.masksOn = True
            self.MCheckBox.setChecked(True)
            self.update_layer()
            self.update_plot()
        ...
        self.ModelChooseB.setEnabled(True)

    def analyze_selected_image_file(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "Select image to analyze", "", "Images (*.tif *.tiff *.ome.tif);;All files (*)")
        if filename and os.path.exists(filename):
            self.pending_analysis_folder = None
            self.analyze_image_file(filename)
            for button in self.punctaStyleButtons:
                button.setEnabled(True)
            print(f"Loaded image for analysis: {filename}. Click Run to start analysis.")

    def analyze_selected_image_folder(self):
        directory = QFileDialog.getExistingDirectory(None, "Select image folder", options=QFileDialog.ShowDirsOnly)
        if directory is not None and os.path.exists(directory) and os.path.isdir(directory):
            first_image = self.get_first_image_in_folder(directory)
            if first_image is None:
                print(f"No TIFF image found in selected folder: {directory}")
                return
            self.analyze_image_file(first_image)
            self.pending_analysis_folder = directory
            for button in self.punctaStyleButtons:
                button.setEnabled(True)
            print(f"Selected image folder for analysis: {directory}. Showing first image: {first_image}. Click Run to start batch analysis.")

    def get_first_image_in_folder(self, directory):
        image_exts = ('.tif', '.tiff', '.ome.tif')
    ...
    def analyze_image_file(self, filename):
        logger.info(f"loading selected image file for analysis {filename}")
        io._load_image(self, filename, load_seg=False)

    def analyze_image_folder(self, directory):
        image_exts = ('.tif', '.tiff', '.ome.tif')
        processed = 0
        failed = 0
        self.processing_analysis_folder = True
        try:
            for root, dirs, files in os.walk(Path(directory)):
                for file in sorted(files):
                    ...
                    logger.info(f"batch processing {filename}")
                    try:
                        io._load_image(self, filename, load_seg=False)
                        self.puncta_detection()
                        processed += 1
                    except Exception as e:
                        failed += 1
                        logger.warning(f"batch analysis failed for {filename}: {e}")
                        print(f"WARNING: batch analysis failed for {filename}: {e}")
        finally:
            self.processing_analysis_folder = False
        print(f"Analysis folder done. Processed {processed} image(s), failed {failed}.")

    def batch_puncta_detection(self):
        self.analyze_selected_image_folder()

    def automatic_imaging(self):
        if not hasattr(self.agent, 'core'):
            print('WARNING: Micro-Manager is not connected; acquisition was not started.')
            return
        # self.
        # self.imagingBox.setEnabled(False)
        for widget in self.imaging_channel_comboboxs:
            widget.setEnabled(False)
        for widget in self.imaging_exposure_comboboxs:
            widget.setEnabled(False)
        self.channel_group_down.setEnabled(False)
        self.timeBox.setEnabled(False)
        self.zStackBox.setEnabled(False)
        self.use_micromanager.setEnabled(False)
        self.file_directory_text.setEnabled(False)
        self.file_directory_button.setEnabled(False)
        self.file_name_text.setEnabled(False)
        ...
        self.agent = SmartMicroscope(self)
        self.agent.close = False
        if self.imaging_thread is not None and self.imaging_thread.isRunning():
            self.imaging_thread.quit()
            self.imaging_thread.wait()
        self.imaging_thread = QThread()
        self.agent.moveToThread(self.imaging_thread)
        if not self.use_micromanager.isChecked():
            if self.use_timelapse.isChecked():
                logger.info('time lapse imaging')
                self.imaging_thread.started.connect(self.agent.time_lapse_imaging)
            else:
                logger.info('2D imaging')
                self.imaging_thread.started.connect(self.agent.imaging)
        else:
            if self.agent.acq_settings.use_frames():
                logger.info('time lapse imaging')
                self.imaging_thread.started.connect(self.agent.time_lapse_imaging)
            else:
                logger.info('2D imaging')
                self.imaging_thread.started.connect(self.agent.imaging)
        self.agent.snap_img_signal.connect(self.update_snap_image)
        self.agent.snap_mask_signal.connect(self.update_snap_mask)
        self.agent.finished_signal.connect(self.finish_imaging)
        self.imaging_thread.start()

    def abort_imaging(self):
        self.agent.close = True
        if self.imaging_thread is not None:
            self.imaging_thread.quit()
            self.imaging_thread.wait()
        self.finish_imaging(None)
        # self.agent.queue.put((None, None, None, None, (None, None, None, None), None, None, None, None))
        # self.analyzer.join()

    def finish_imaging(self, a0):
        logger.info('finish imaging')
        if not self.use_micromanager.isChecked():
            for widget in self.imaging_channel_comboboxs:
            ...
            self.file_directory_button.setEnabled(True)
            self.file_name_text.setEnabled(True)
        self.use_micromanager.setEnabled(True)
        ...
        self.segBox.setEnabled(True)
        self.abortButton.setEnabled(False)
        self.agent.imaged_area_tracker.clear()
        self.agent.snapped_area_tracker.clear()

    def update_snap_image(self, image):
        self.reset()
        initialize_images(self, image, load_3D=False)

    def update_snap_mask(self, mask):
        io._masks_to_gui(self, mask, outlines=None)
        self.masksOn = True
        self.MCheckBox.setChecked(True)

    def load_settings(self):
        settings_path = Path(os.environ.get(
            'CELLQUANT_SETTINGS_PATH', Path.home() / '.cellquant' / 'settings.json'))
        if settings_path.exists():
            ...
            with open(settings_path, 'r') as fp:
                record = json.load(fp)
                self.use_timelapse.setChecked(record['use T'])
                self.time_duration_text.setText(record['time duration'])
                self.time_interval_text.setText(record['time interval'])
                self.channel_group_down.setCurrentText(record['channel group'])
                for i in range(len(self.imaging_channel_comboboxs)):
                    self.imaging_channel_comboboxs[i].setCurrentText(record['channels'][i])
                    self.imaging_exposure_comboboxs[i].setText(record['exposures'][i])
                self.file_directory_text.setText(record['file directory'])
                self.file_name_text.setText(record['file name'])
                self.find_cell_combox.setCurrentText(record['find cell chan'])
                self.cellNumberEdit.setText(record['cell number'])
                self.ModelChooseB.setCurrentText(record['segmentation model'])
                self.punctaThreshText.setText(record['puncta threshold'])
                if hasattr(self, 'analysis_instruction_text'):
                    self.analysis_instruction_text.setText(record.get('analysis instruction', ''))
                if hasattr(self, 'use_language_instruction'):
                    self.use_language_instruction.setChecked(record.get('use language instruction', False))
                if hasattr(self, 'custom_analysis_mode'):
                    self.custom_analysis_mode.setCurrentText(record.get('custom analysis mode', 'LLM Python script'))
                for i in range(len(self.punctaChannelChoose)):
                    self.punctaChannelChoose[i].setCurrentText(record['puncta channels'][i])
                self.useDL.setChecked(record['use DL'])
    ...
    def closeEvent(self, event):
        if self.imaging_thread is not None:
            ...
            self.imaging_thread.wait()
        self.agent.close = True
        self.queue.put((None, None, None, None, (None, None, None, None), None, None, None, None, None, None))
        self.writer_queue.put((None, None, None, None))
        self.plot_queue.put((None, None, None, None, None))
        # record the current settings
        if not self.use_micromanager.isChecked():
            record = {'use T': self.use_timelapse.isChecked(), 'time duration': self.time_duration_text.text(),
                      'time interval': self.time_interval_text.text(),
                      'use Z': self.use_zstack.isChecked(), 'channel group': self.channel_group_down.currentText(),
                      'channels': [self.imaging_channel_comboboxs[0].currentText(),
                                   self.imaging_channel_comboboxs[1].currentText(),
                                   self.imaging_channel_comboboxs[2].currentText(),
                                   self.imaging_channel_comboboxs[3].currentText()],
                      'exposures': [self.imaging_exposure_comboboxs[0].text(),
                                    self.imaging_exposure_comboboxs[1].text(),
                                    self.imaging_exposure_comboboxs[2].text(),
                                    self.imaging_exposure_comboboxs[3].text()],
                      'file directory': self.file_directory_text.text(), 'file name': self.file_name_text.text(),
                      'find cell chan': self.find_cell_combox.currentText(), 'cell number': self.cellNumberEdit.text(),
                      'segmentation model': self.ModelChooseB.currentText(),
                      'puncta threshold': self.punctaThreshText.text(),
                      'analysis instruction': self.get_analysis_instruction_text(),
                      'use language instruction': self.use_language_instruction.isChecked() if hasattr(self, 'use_language_instruction') else False,
                      'custom analysis mode': self.get_custom_analysis_mode(),
                      'puncta channels': [self.punctaChannelChoose[i].currentText() for i in
                                          range(len(self.punctaChannelChoose))], 'use DL': self.useDL.isChecked()}
            import json
            settings_path = Path(os.environ.get(
                'CELLQUANT_SETTINGS_PATH', Path.home() / '.cellquant' / 'settings.json'))
            try:
                ...
                logger.warning(f'Could not save settings to {settings_path}: {e}')
        for process in (self.analyzer, self.writer_process, self.plot_process):
            process.join(timeout=5)
            if process.is_alive():
                logger.warning(f'Terminating unresponsive background process: {process.name}')
                process.terminate()
                process.join(timeout=1)
        if hasattr(self.agent, 'core'):
            self.agent.core._close()

    def level_change(self, r):
        r = ["C1", "C2", "C3", "C4"].index(r)
        if self.loaded:
    ...
    def check_gpu(self, torch=True):
        # also decide whether to use torch
    ...
    def get_channels(self):
        channels = [
            self.ChannelChoose.currentIndex(), 0
        ]
        ...
        if self.nchan == 1:
            ...
            if channels[0] == 3 or channels[0] == 4:
                channels[0] = 0
                print(
                    f"GUI_WARNING: only two channels in image, cannot use blue channel, changing channels"
                )
                logger.info(  f"GUI_WARNING: only two channels in image, cannot use blue channel, changing channels")
        elif self.nchan == 3:
            if channels[0] == 4:
                channels[0] = 0
    ...
    def model_choose(self, custom=False):
        ...
        if index >= 0:
            custom = False
            if index >= len(self.net_names):  # is the custom model
                model_name = self.ModelChooseB.currentText()
                custom = True
            else:
                model_name = self.net_names[index]
            display_name = self.ModelChooseB.currentText()
            print(f"GUI_INFO: selected model {display_name}, loading now")
            logger.info(f"GUI_INFO: selected model {display_name}, loading now")
            ...
            logger.info(f"GUI_INFO: diameter set to {self.diameter: 0.2f} (but can be changed)")

    def calibrate_size(self):
        try:
            calibration_model = models.Cellpose(gpu=self.useGPU.isChecked(), model_type='cyto3')
            diams, _ = calibration_model.sz.eval(self.stack[self.currentZ].copy(),
                                                 channels=self.get_channels(),
                                                 progress=self.progress)
        except Exception as e:
            print(f'WARNING: cell-size calibration is unavailable: {e}')
    ...
    def enable_buttons(self):
        ...
        self.ImagingButton.setEnabled(True)
        for i in range(len(self.punctaStyleButtons)):
            self.punctaStyleButtons[i].setEnabled(True)
        for n in range(self.nchan):
            self.sliders[n].setEnabled(True)
        for n in range(self.nchan, 4):
            self.sliders[n].setEnabled(True)

        self.toggle_mask_ops()

        self.update_plot()
        self.setWindowTitle(f"Micro-copilot — {self.filename}")
    ...
    def disable_buttons_removeROIs(self):
        ...
        self.run_segmentation_button.setEnabled(False)
        for i in range(len(self.punctaStyleButtons)):
            self.punctaStyleButtons[i].setEnabled(False)
    ...
    def remove_action(self):  # used in menu
        if self.selected > 0:  # is updated in select_cell function and is used in the guiparts mouse click event and
    ...
    def dropEvent(self, event):
        ...
        if not files or not files[0]:
            event.ignore()
            return
        if os.path.splitext(files[0])[-1].lower() == ".npy":
    ...
    def toggle_masks(self):
        ...
        if not self.masksOn and not self.outlinesOn:
            self.p0.removeItem(self.layer)  # layer is used to show the mask
            self.layer_off = True
        else:
            if self.layer_off:
    ...
    def reset(self):
        ...
        self.stack_unnorm = np.zeros((1, self.Ly, self.Lx, 3))
    ...
    def select_cell(self, idx):  # is used in the guiparts mouse click event
    ...
    def remove_cell(self, idx):  # the idx of all other cells should minus 1
    ...
    def remove_single_cell(self, idx):
        ...
        self.selected = 0
        if self.NZ > 1:  # NZ is the number of the z layers
    ...
    def undo_remove_cell(self):  # the undo cell idx is latest
    ...
    def remove_stroke(self, delete_points=True,
                      stroke_ind=-1):  # not understood yet, seems like to remove the stroke during the drawing
    ...
    def update_plot(self):  # update the image, including the saturation, show the image or optical flow...
        ...
        if self.view == 0 or self.view == self.ViewDropDown.count() - 1:
            ...
            if self.color == 0:
                if self.nchan > 1:
                    img_rgb = np.zeros((image.shape[0], image.shape[1], 3))
                    for i in range(self.nchan):
                        img_single = image[..., i]
                        low, high = self.saturation[i][self.currentZ]
                        scale = high - low
                        img_single = np.zeros_like(img_single) if scale <= 0 else (img_single - low) / scale
                        img_rgb += img_single[..., np.newaxis] * np.array(self.lut_color[i])[np.newaxis, np.newaxis, :]
                        self.img.setLevels([0, 255])
                    image = np.clip(img_rgb, a_min=0, a_max=255).astype(np.uint8)
                ...
                self.img.setImage(image, autoLevels=False, lut=self.cmap[self.color])  # todo: update cmap and lut
                if self.nchan > 1:
    ...
    def update_layer(self):  # update the segmentation layer, the segmentation layer is overlap with the image layer
        if self.masksOn or self.outlinesOn:
            # self.draw_layer()
            self.layer.setImage(self.layerz, autoLevels=False)  # the layerz is updated on draw_mask function
    ...
    def add_set(self):  # used by the guipart after the draw
        if len(self.current_point_set) > 0:
    ...
    def add_mask(self, points=None, color=(100, 200, 50), dense=True):
        ...
        ars, acs, vrs, vcs = np.zeros(0, "int"), np.zeros(0, "int"), np.zeros(
            0, "int"), np.zeros(0, "int")  # ar ac is the area pixel and vr, vc is the outline pixel
    ...
    def update_time(self):
        self.time_duration = int(self.time_duration_text.text())
        self.time_interval = int(self.time_interval_text.text())

    def update_zstack_zstart(self):
        if hasattr(self.agent, 'core'):
            self.z_start_label.setText(str(round(self.agent.core.get_position(), 3)))
        else:
            print('WARNING: connect Micro-Manager before setting the Z-stack start position.')

    def update_zstack_zend(self):
        if hasattr(self.agent, 'core'):
            self.z_end_label.setText(str(self.agent.core.get_position()))
        else:
            print('WARNING: connect Micro-Manager before setting the Z-stack end position.')
    ...
    def set_normalize_params(self, normalize_params):
        from cellquant.models import normalize_default
    ...
    def compute_saturation(self, return_img=False):  # smooth and sharpen the image,
        ...
        if len(self.saturation) < 4:
            for i in range(4 - len(self.saturation)):
                self.saturation.append([])
                for n in range(self.NZ):
                    self.saturation[-1].append([0, 255.])
    ...
    def get_model_path(self, custom=False):
        if custom:
            ...
            self.current_model = self.net_names[max(0, self.ModelChooseB.currentIndex())]
            if self.current_model == models.CELLQUANT_MODEL_NAME:
                self.current_model_path = models.default_cellpose_model_path()
            else:
    ...
    def initialize_model(self, model_name=None, custom=False):
        if model_name == "dataset-specific models":
            ...
            if self.current_model == models.CELLQUANT_MODEL_NAME:
                self.current_model_path = models.default_cellpose_model_path()
            elif self.current_model == "cyto" or self.current_model == "nuclei":
                self.current_model_path = models.model_path(self.current_model, 0)
            else:
                self.current_model_path = os.fspath(
                    models.MODEL_DIR.joinpath(self.current_model))
        multi_scale = False
        if 'multiscale' in self.current_model_path:
            multi_scale = True
            print('Using multiscale model')
        if custom or self.current_model == models.CELLQUANT_MODEL_NAME or model_name is None or not isinstance(model_name, str):
            self.model = models.CellposeModel(gpu=self.useGPU.isChecked(),
                                              pretrained_model=self.current_model_path, multi_scale=multi_scale)
        else:
            if self.current_model != "cyto3":
                diam_mean = 17. if self.current_model == "nuclei" else 30.
                self.model = models.CellposeModel(gpu=self.useGPU.isChecked(),
                                                  diam_mean=diam_mean,
                                                  model_type=self.current_model, multi_scale=multi_scale)
            else:
                self.model = models.Cellpose(gpu=self.useGPU.isChecked(),
                                             model_type=self.current_model, multi_scale=multi_scale)
        diam_labels = float(np.asarray(self.model.diam_labels).squeeze())
        if np.isfinite(diam_labels) and diam_labels > 0:
            self.diameter = diam_labels
            self.Diameter.setText(f"{diam_labels:.2f}")
    ...
    def train_model(self, restore=None, normalize_params=None):
        ...
        self.logger.info(
            f"training with chan = {self.ChannelChoose.currentText()}"
        )
    ...
    def compute_segmentation(self, custom=False, model_name=None):
        self.progress.setValue(0)
        if not hasattr(self, 'current_model_path'):
            self.initialize_model(model_name=models.CELLQUANT_MODEL_NAME, custom=False)
        if 1:
            tic = time.time()
            self.clear_all()
            self.flows = [[], [], []]
    ...
    def cell_segmentation_resize(self):
        tic = time.time()
        self.clear_all()
        self.flows = [[], [], []]
        if not hasattr(self, 'current_model_path'):
        ...
        if self.restore is not None and self.restore != "filter":
            ...
            img = self.stack.copy().squeeze()
        image = cv2.resize(img, None, fx=1. / self.agent.resize_scale, fy=1. / self.agent.resize_scale,
                           interpolation=cv2.INTER_LINEAR)
        # min_size = 0
        min_size = self.agent.cell_min_size // (self.agent.resize_scale ** 2)
        channels = self.get_channels()
        flow_threshold, cellprob_threshold = self.get_thresholds()
        self.diameter = float(self.Diameter.text())
        # diameter = self.diameter / self.agent.resize_scale
        ...
        masks = cv2.resize(masks, None, fx=self.agent.resize_scale, fy=self.agent.resize_scale,
                           interpolation=cv2.INTER_NEAREST)
        flows_new = []
        flows_new.append(flows[0].copy())  # RGB flow
        flows_new.append((np.clip(normalize99(flows[2].copy()), 0, 1) *
