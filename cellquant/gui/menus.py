...
def analysismenu(parent):
    ...
    parent.batch_analysis = QAction("Batch organelle detection", parent)
    # parent.addmodel.setShortcut("Ctrl+A")
    parent.batch_analysis.triggered.connect(parent.batch_puncta_detection)
    parent.batch_analysis.setEnabled(True)
    analysis_menu.addAction(parent.batch_analysis)

    parent.open_gpt_dialog_action = QAction("Open Co-Scientists dialog", parent)
    parent.open_gpt_dialog_action.setShortcut("Ctrl+Shift+G")
    parent.open_gpt_dialog_action.triggered.connect(parent.open_gpt_dialog)
    parent.open_gpt_dialog_action.setEnabled(True)
    analysis_menu.addAction(parent.open_gpt_dialog_action)
