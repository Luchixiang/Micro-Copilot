...
class punctaModelButton(QPushButton):
...
class TrainWindow(QDialog):
    ...
    def accept(self, parent):
        # set channels
        for i in range(2):
            parent.ChannelChoose.setCurrentIndex(
...
class HelpWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Micro-copilot help")
        self.win = QWidget(self)
        layout = QGridLayout()
        self.win.setLayout(layout)
...
class ImageDraw(pg.ImageItem):
    ...
    **Bases:** :class:`GraphicsObject <pyqtgraph.GraphicsObject>`
    ...
    def mouseClickEvent(self, ev):
        if (self.parent.masksOn or
            ...
            if self.parent.loaded \
                    ...
                    and not self.parent.deleting_multiple:  # the image is loaded, starting drawing the mask
                if not self.parent.in_stroke:
                    ...
                    self.end_stroke()
                    self.parent.in_stroke = False
            elif not self.parent.in_stroke:  # select cell
                    ...
                    if ev.button() == QtCore.Qt.LeftButton and not ev.double():
                        ...
                        if idx > 0:
                            if ev.modifiers() & QtCore.Qt.ControlModifier:
                                ...
                                self.parent.remove_cell(idx)  # control delete directly
                            elif ev.modifiers() & QtCore.Qt.AltModifier:
                                self.parent.merge_cells(idx)  # alt merge
                            elif self.parent.masksOn and not self.parent.deleting_multiple:
                                self.parent.unselect_cell()
                                self.parent.select_cell(idx)  # select cell
                            elif self.parent.deleting_multiple:
                                if idx in self.parent.removing_cells_list:  # multiple deleting
    ...
    def end_stroke(self):  # is the end, add mask and save, auto connect the end point with the start point
        self.parent.p0.removeItem(self.scatter)
        if not self.parent.stroke_appended:
            self.parent.strokes.append(self.parent.current_stroke)  # add strokes
    ...
    def drawAt(self, pos,
               ev=None):  # when you are drawing the mask, add the dots to the position and add stroke to current_stroke
        ...
        for ky, y in enumerate(np.arange(ty[0], ty[1], 1, int)):
            for kx, x in enumerate(np.arange(tx[0], tx[1], 1, int)):
                ...
                stroke.append([self.parent.currentZ, x, y, iscent])  # update self.parent.current_stroke
        self.updateImage()
    ...
    def setDrawKernel(self, kernel_size=3):
        ...
        onmask = 255 * kernel[:, :, np.newaxis]  # 3 x 3 x 4
        offmask = np.zeros((bs, bs, 1))
        opamask = 100 * kernel[:, :, np.newaxis]  # RGBA channel
