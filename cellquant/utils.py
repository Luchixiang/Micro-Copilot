...
from cellquant.background_estimation import background_estimation

from PIL import Image

def normalize_mask(img, cell_mask,minv=None,maxv=None, return_minmax=False):
    cell_image = np.ma.masked_where(cell_mask == 0, img)
    cell_image = np.ma.filled(cell_image, np.nan)
    if minv is None and maxv is None:
        minv = np.nanpercentile(cell_image, 1)
        maxv = np.nanpercentile(cell_image, 99)
        # print('min max:', minv, maxv, np.nanpercentile(cell_image, 99.9))
        # if maxv - minv < 10:
        #     # minv = np.nanpercentile(cell_image, 1)
        #     maxv = np.nanpercentile(cell_image, 99.9)
    original_image = img.astype(np.float32)
    original_image = (original_image -minv) / (
            maxv - minv) * 255
    original_image[original_image < 0] = 0
    original_image[original_image > 255] = 255
    if return_minmax:
        return original_image.astype(np.uint8), minv, maxv
...
def ema_normalize_mask(img, cell_mask,ema_minv=None,ema_maxv=None):
    cell_image = np.ma.masked_where(cell_mask == 0, img)
    cell_image = np.ma.filled(cell_image, np.nan)

    minv = np.nanpercentile(cell_image, 1)
    maxv = np.nanpercentile(cell_image, 99)
    # if ema_minv is not None:
    #     minv = 0.1 * minv + 0.9 * ema_minv
    #     maxv = 0.1 * maxv + 0.9 * ema_maxv
    # print('min max:', minv, maxv, np.nanpercentile(cell_image, 99.9))
    original_image = img.astype(np.float32)
    original_image = (original_image - minv) / (
            maxv - minv) * 255
...
def fit_to_gaussian_nuclear(img, nuclear_mask, cell_mask):
    img = img[np.where((cell_mask != 0) & (nuclear_mask != 0))]
...
def fit_to_gaussian(img, nuclear_mask, cell_mask):
    img = img[np.where((cell_mask != 0) & (nuclear_mask == 0))]
...
def change_to_8bit(img):
    scale = 256.0 / (img.max() - img.min() + 1)
    value = np.bitwise_and(img, 0xffff) - img.min()
    value[value < 0] = 0
    value = (value * scale + 0.5).astype(np.uint16)
    value[value > 255] = 255
    value = value.astype(np.uint8)
    return value


def normalize(img, cell_image):
    original_image = img.astype(np.float32)
    original_image = (original_image - np.percentile(cell_image, 1)) / (
            np.percentile(cell_image, 99) - np.percentile(original_image, 1)) * 255
    original_image[original_image < 0] = 0
    original_image[original_image > 255] = 255
...
def get_unique_filename(dir, filename):
    ...
    counter = 0
    while os.path.exists(os.path.join(dir, f"{base}_{counter}{ext}")):
        counter += 1
    return os.path.join(dir, f"{base}_{counter}{ext}")
